...
const App = () => {
  ...
  const [hapticsEnabled] = useState(hapticFeedback.isSupported());
  ...
  useEffect(() => {
    if (currentLevel.number > previousLevel) {
      ...
      // Trigger level up haptic feedback
      if (hapticsEnabled) {
        hapticFeedback.levelUp();
      }
      setTimeout(() => setShowLevelUp(false), 1500);
    }
  }, [currentLevel.number, previousLevel, hapticsEnabled]);
    ...
    <div className="bg-gradient-main min-h-screen px-4 flex flex-col items-center text-white font-medium">
      ...
      <div className="w-full z-10 min-h-screen flex flex-col items-center text-white">
        ...
        <div className="fixed top-0 left-0 w-full px-4 pt-8 z-10 flex flex-col items-center text-white">
          ...
          <div className="mt-12 text-5xl font-bold flex items-center flex-col">
        ...
        <div className="flex-grow flex flex-col items-center justify-center gap-6">
          <div className="relative" onClick={handleClick}>
            <img 
              ...
              className={`coin-transition ${showLevelUp ? 'level-up-animation' : ''}`}
            />
            {clicks.map((click) => (
              <div
                key={click.id}
          ...
          <div className="flex flex-col items-center">
            <div className={`text-xl level-indicator ${showLevelUp ? 'level-up-animation' : ''}`}>
              Level {currentLevel.number}
              {showLevelUp && (
                <span className="ml-2 text-yellow-300">Level Up!</span>
              )}
            </div>
            {currentLevel.number < 6 && (
              <div className="text-base mt-2 next-level-indicator">
                Next level at {levels[currentLevel.number].requiredPoints.toLocaleString()} coins
              </div>
            )}
          </div>
        </div>
